...
*   ********************
    ...
    while(run)
      ...
      'define td=(tc-((100-RHprs)/5))'
      'define wspd=(sqrt(UGRDprs*UGRDprs + VGRDprs*VGRDprs))*1.89'
      'define wdir=(atan2(UGRDprs,VGRDprs))*57.2957795+180.0'
      Col=1
      thick=15

      rc=plotskew(tc,td,wspd,wdir,0,Col,Col,Col,Col,overlay,thick,thick,thick)
...
*   ** DRAW TIME ***
    ...
    "set string 1 c 6"
    "set strsiz 0.08"
    'draw string 10 0.1 Valid: 'vhr':'vmin'z: 'vmonth' 'vday', 'vyr
*   ****************
    ...
    'set string 255 bl 6'
    'set strsiz 0.08'
    'draw string 0.1 0.1 http://derecho.math.uwm.edu/~bmburlin/'
    'set strsiz 0.12'
    'set string 1 c 6'
    'draw string 5.8 8.2 'city', 'state
    'set string 1 r 1'
    'set strsiz 0.1'
    'draw string 10.95 8.4 Rapid Refresh (RAP) 13km'
    'draw string 10.95 8.2 Initialized: 'inithr': 'initmonth' 'initday', 'inityr

    'gxprint skewt_'city'_'t-1'.png x800 y600'
    'c'
    t=t+1
  endwhile
...
*** FINISH SKEW-T ***
*********************
'!mv skewt_*png /tornado/r1/bmburlin/public_html/graphics/'model'/skewt'
'quit'
...
*************************************************************************
function plotskew(sndtemp,snddewp,sndspd,snddir,ClrScrn,TSndCol,DSndCol,PrclCol,RHCol,OL,TSndThk,DSndThk,RHThk)
*************************************************************************
...
If (DrawTemp = 1)
   ...
   If (LblAxes = 1) 
      ...
      Else
         "draw string 5.3 0.15 Temperature (`3.`0C)"
      Endif
   Endif
Endif
...
If (DrawMix = 1)
   ...
   If (LblAxes = 1)
      ...
      Else
         "draw string 9.65 4 Mixing Ratio (g/kg)"
      Endif
      "set string 1 c 3 0"
   Endif
Endif
